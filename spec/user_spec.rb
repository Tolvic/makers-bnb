...
describe User do
  ...
  describe '#create' do
    it 'creates a new user' do
      ...
      expect(@user.email_address).to eq 'IhatedaffyD123'
    end
  end
  ...
  describe '#find' do
    it 'finds a user by ID' do
      ...
      expect(result.email_address).to eq @user.email
    end
  ...
  describe '#authenticate' do
    it 'returns nil given an incorrect email address' do
      expect(User.authenticate(email: 'nottherightemail@me.com', password: 'IhatedaffyD123')).to be_nil
    end

    it 'returns nil given an incorrect password' do
      expect(User.authenticate(email: 'DDowd@duckmail.com', password: 'wrongpassword')).to be_nil
    end

    it 'returns a user given a correct username and password, if one exists' do

      authenticated_user = User.authenticate(email: 'DDowd@duckmail.com', password: 'IhatedaffyD123')
